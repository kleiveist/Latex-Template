...
Das aktive Manifest heißt \texttt{project-profile.toml}. Es wird für jedes
generierte Projekt erzeugt. \texttt{schema\_version} hält die Schema-Version
fest. \texttt{id}, \texttt{name} und \texttt{description} beschreiben das
Profil. \texttt{optional\_features} erfasst die angeforderten Erweiterungen;
\texttt{features} enthält die vollständige Auflösung. Das Tooling lädt und
validiert das Manifest. Dadurch behandelt es nur aktive Dienste und Workflows.
Im Master nennt das Manifest \texttt{full-platform} als Profil. Zusätzlich
aktiviert es \texttt{postgres}; das Preset selbst bleibt davon unberührt.
Profilstruktur und Laufzeitwerte sind getrennte Achsen. Development-, Test- und
